...
class ProductViewTest(TestCase):
    def test_get_product(self):
        ...
        product_1 = ProductProxy.objects.create(title='Product 1', category=category, image=uploaded, slug='product-1')
        product_2 = ProductProxy.objects.create(title='Product 2', category=category, image=uploaded, slug='product-2')
...
class ProductDetailViewTest(TestCase):
    def test_get_product_by_slug(self):
        ...
        category = Category.objects.create(name='Category 1')
        product = ProductProxy.objects.create(title='Product 1', category=category, slug='product-1', image=uploaded)
        # Make a request to the product detail view with the product's slug
        response = self.client.get(reverse('shop:product_detail', kwargs={'product_slug': 'product-1'}))
