...
class Order(models.Model):
    ...
    def get_discount(self):
        if (total_cost := self.get_total_cost_before_discount()) and self.discount > 0:
            return total_cost * (self.discount / Decimal(100))
        else:
            return Decimal(0)

    def get_total_cost_(self):
...
class OrderItem(models.Model):
    ...
    class Meta:
        ...
        ordering = ['-id']
        constraints = [
            models.CheckConstraint(check=models.Q(quantity__gt=0), name='quantity_gte_0'),
        ]
